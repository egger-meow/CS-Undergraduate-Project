...
**本研究基於參與電梯異常檢測專案的經驗，取用專案中學長姐蒐集的電梯實驗資料進行實驗與方法研究。**


* **目標**：偵測「再平層」事件（電梯到站第一次對齊後，因誤差過大而進行第二次對齊），以降低人工巡檢負擔。


* **資料來源**：使用專案中蒐集的馬達電流（channel 0）+ 六軸振動（門上 XYZ：channels 1–3；車廂/馬達背面 XYZ：channels 4–6）資料，原始取樣率 8192 Hz，訓練前下採樣至約 256 Hz；實驗使用 0604–0605 的正常資料與 0607 的異常資料；切片聚焦 **Stage 3–5**。
...
> 基於實驗資料分析發現：**門上 XYZ 資訊性高，車廂/馬達背面 XYZ 資訊性低**。
...
基於學長姐蒐集的資料進行劃分，設定為 **Train/Validate ＝ 85%/15%**，並列出數量：
...
* **次佳或不佳**：`[0,1,2,3]` 、`[2,3]` 、`[0,2,3]` 

 等常 **≤ ~0.7 accuracy**（以相同的 train/test 集下測得）。
...
**說明：本人未參與實體電梯實驗設計與資料蒐集，以下貢獻皆基於使用學長姐蒐集的實驗資料進行的演算法開發與分析。**
